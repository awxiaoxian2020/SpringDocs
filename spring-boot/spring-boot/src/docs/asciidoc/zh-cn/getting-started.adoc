...
* 我们它确信能够开箱即用，但如果这一方式无法满足你的需求，你也可以迅速地自定义配置。
* 提供大量非功能性特性相关项目 (例如: 内嵌服务器、安全、指标、健康检查、外部配置).
* 绝对没有代码生成,也不要求 XML 配置.

[[getting-started-system-requirements]]
== 系统要求
Spring Boot {spring-boot-version} 需要 https://www.java.com[Java 8] 并且与 Java 16 (包括) 兼容.  还需要 {spring-framework-docs}/[Spring Framework {spring-framework-version}]  或更高版本.
...
=== 安装 Spring Boot CLI
Spring Boot CLI 是一个命令行工具,如果您想使用 Spring 快速搭建项目原型,您可以选择它. 它允许您运行 https://groovy-lang.org/[Groovy] 脚本,这意味着您有可以有类 Java 语法且没有太多样板的代码.
